...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', ADENIT_SCRIPTS .'/post-like.js', array('jquery'), '1.0', 1 );
...
function getPostLikeLink( $post_id ) {
	...
	$output = '<a href="#" class="jm-post-like'.$class.'" data-post_id="'.$post_id.'" title="'.esc_attr( $title ).'">'.$count.'&nbsp;'.$heart.'</a>';
	return $output;
}
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php esc_html_e( 'You Like:' , 'aden' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . esc_html__( 'You don\'t like anything yet.', 'aden' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__( 'You Like:', 'aden' ) . "</h3>\n";
			$like_list .= "<ul>\n";
